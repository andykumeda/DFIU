...
export function LoginForm() {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    if (error) {
      setError('We could not send the reset email. Please check the address and try again.')
      return
    }
    setResetSent(true)
  }
  ...
  if (forgotPassword) {
    return (
      <form onSubmit={handlePasswordReset} className='bg-neutral-900 border border-neutral-800 p-8 rounded-xl w-full max-w-md'>
        <h1 className='text-2xl font-bold text-white mb-2'>Reset your password</h1>
        <p className='text-neutral-400 mb-6'>Enter your email and we&apos;ll send you a link to choose a new password.</p>
        ...
        <div>
          <label htmlFor='reset-email' className='block text-sm font-medium text-neutral-300 mb-1'>Email</label>
          <input
            id='reset-email'
            type='email'
            value={email}
        ...
        <button
          type='submit'
          disabled={loading}
          className='w-full mt-5 bg-blue-600 hover:bg-blue-500 text-white font-semibold py-2 px-4 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
        >
        ...
        <button
          ...
          className='w-full mt-3 text-blue-500 hover:text-blue-400 font-medium text-sm'
        >
          Back to sign in
        </button>
      </form>
