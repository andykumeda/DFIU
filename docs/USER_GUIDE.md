...
Use **Edit Template** to change the shared checklist or add labeled text fields with default text, such as Notes. You can click the nearby add button for each new item or field, or enter it and click **Save Template** directly. Each text field appears in every bag and in Crew View, and can be changed for one bag. A normal **Save Template** applies template additions, removals, renames, and new defaults while retaining matching checked states, quantities, custom items, and per-bag text edits. The bag card and **What's Inside** view list packed items; open **Edit Drop Bag** to see all template checklist items, including unchecked ones. Use **Save & Replace All Bags** when you want the template to start every bag over: after confirmation it clears checked states, quantities, custom items, per-bag item edits, and template text field edits across Start, Finish, official, and crew bags. Bag names and bag notes remain. When adding a custom item inside one bag, choose the category where it should appear.
